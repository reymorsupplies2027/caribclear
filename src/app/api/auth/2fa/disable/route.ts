...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!result.ok) return fail(401, 'INVALID_CODE', 'Confirm with a current code to disable 2FA.');
    await db.userSecurity.update({
      where: { userId: s.userId },
      data: { totpEnabled: false, totpSecretEnc: null, backupCodesHash: null, lastTotpCounter: null },
    });
    await appendAuditLog({ tenantId: s.tenantId, userId: s.userId, action: 'security.2fa_disabled', entityType: 'user', entityId: s.userId });
